...
class Manager {
    ...
    protected static function createRequest(\User\Model $Owner, $data, $requestType, $origin = RequestModel::ORIGIN_USER) {
        ...
        switch ($requestType) {
            case \SPZ\RequestModel::REQUEST_TYPE_SPART:
                if (!empty($data['parts'])) {
                    $RequestModel->setMasterUnits($data['parts']);
                }
                break;
            case \SPZ\RequestModel::REQUEST_TYPE_CARSERVICE:
    ...
    //создать запрос на подбор запчастей
    public static function createSPartRequest(\User\Model $Owner, $data) {
        return static::createRequest($Owner, $data, RequestModel::REQUEST_TYPE_SPART);
    }

    //создать запрос на сервис
    public static function createCarserviceRequest(\User\Model $Owner, $data) {
        return static::createRequest($Owner, $data, RequestModel::REQUEST_TYPE_CARSERVICE);
    }

    //создать коснультацию
    public static function createConsult($User, $Company, $initData = []) {

        //особый код, отличающийся от static::createRequest
    ...
    public static function transferRequests(\User\Model $Recipient, \User\Model $Donor) {
        $donorId = $Donor->id();
        $recipientId = $Recipient->id();

        \Project\Logger::debugTransferText(sprintf("Starting to transfer requests, recipientId: %d, donorId: %d", $recipientId, $donorId));

        $DonorShardDAO = ShardDAO::getInstance($donorId);
        $RecipientShardDAO = ShardDAO::getInstance($recipientId);

        $DonorShardConnection = $DonorShardDAO->getShardConnection();
        $RecipientShardConnection = $RecipientShardDAO->getShardConnection();

        try {
            $DonorShardConnection->startTransaction();
            $RecipientShardConnection->startTransaction();
        } catch(\Db\Sql\Exception_TransactionAlreadyStarted $E) {}

        try {
            // МНОГО КОДА
        } catch(\Db\Sql\Exception_Base $E) {
            $DonorShardConnection->rollBack();
            $RecipientShardConnection->rollBack();
            \Project\Logger::debugTransferText(sprintf("Something went wrong in requests transfer, see LOGFILE_APOCALYPSE, recipientId: %d, donorId: %d", $recipientId, $donorId));
            \Project\Logger::logException(\Project\Logger::LOGFILE_APOCALYPSE, $E);
            throw new \Transfer\TransferException('Произошла ошибка при переносе запросов, обратитесь в службу поддержки');
        }
    }
     ...
     * @return RequestModel[]
     */
    public static function getRequestsByUser($Owner) {
        if (!is_object($Owner)) {
            $Owner = \User\Manager::getUserById($Owner, false);
            if (!$Owner) {
                return [];
        ...
        $ownerId = $Owner->id();
        $result = ShardDAO::getInstance($ownerId)->getRequestsByUserIdNStatus();
        foreach ($result as $k => $v) {
            $result[$k] = static::makeRequestObject($v);
        }
    ...
    /**
     * @param int $userId
     * @param bool $getActive `true` - opened requests, `false` - archived requests
     * @param int $offset
     * @param int $limit
     * @return array
     */
    public static function getRequestListByUserId($userId, $getActive = true, $offset = 0, $limit = 0) {

        if ($getActive) { // get active request list, unlimited
            $MemcachedConnection = ShardDAO::getShardMemcachedConnection();
            $list = $MemcachedConnection->get($userId, ShardDAO::MEMCACHED_OPENED_REQUESTS_BY_USER_ID);
            if (!$list) {
                $list = ShardDAO::getInstance($userId)->getRequestsByUserIdNStatus(RequestModel::getOpenStatuses());
                $MemcachedConnection->set($userId, ShardDAO::MEMCACHED_OPENED_REQUESTS_BY_USER_ID, $list);
            }
        } else { // get limited request list
            $limit = $limit > 0 ? $limit : 10;
            $list = ShardDAO::getInstance($userId)->getRequestsByUserIdNStatus(RequestModel::getClosedStatuses(), $offset, $limit);
        }

        foreach ($list as $k => $v) {
            $list[$k] = static::makeRequestObject($v)->data;
            if (isset($list[$k]['vehicleSnapshot'])) {
                $list[$k]['chassisIdTypeName'] = \Translation\Service::tr('vehicle.chassisIdTypes')[$list[$k]['vehicleSnapshot']['chassisIdType']];
            }
        }
        return $list;
    }
    ...
    protected static function searchRequestEntries($managerId, $companyId, $params, $offset, $limit, $orderBy = []) {
        ...
        return static::getSearchDao()->search($criteria, $attr, $isOpen, $isClosed, $limit, $offset, $orderBy);
    }
