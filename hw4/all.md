...
Данный контролер обрабатывает запросы связаные с работой заказа.
Он немного разжирел и если в данный момент проводит его рефакторинг, то можно было бы применить патерн 
"Шаблонный метод", т.к. все операции повторяются.

Также у нас в данном методы есть такие зависимости как: 
