   ...
   private function NewClient($signature=false)
   {
       ...
       if ($signature) {
           ...
           if (DB::query($query, $params)) {
               $this->SetLog("Записали клиента в базу");
   ...
   private function LoadClient(){
        if(!$this->CheckClient()){
            if (isset($_COOKIE["signature"]) && $_COOKIE["signature"]!=""){
                $this->SetLog("Новый клиент с готовой сигнатурой - создаем его в базе ");
                ...
                $this->SetLog("Абсолютно новый клиент - создаем устанавливаем ему куки");
                $this->SetSignatura($_COOKIE["PHPSESSID"]);
            }
        }else{
            $this->SetLog("Готовый клиент - ничего не надо далать");
            return $this->cid;
        }
   }
