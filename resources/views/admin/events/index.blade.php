        ...
        <table class="table table-striped table-sm">
            ...
            @foreach ($eventList as $event)
                <tr>
                    ...
                    <td>
                        <form action="{{route('admin.events.edit', $event)}}" method="GET">
                            {{ csrf_field() }}
                            {{ method_field('PUT') }}
                            <button class="btn btn-sm btn-outline-secondary" value="submit" type="submit">
                    ...
                    <td>{{ $event->is_solved }}</td>
                    <td>{{ $event->getCountryName() }}</td>
                    <td>
                        <a href="{{route('admin.users.show', $event->author_id)}}">
                            {{ $event->getAuthor()->name ?? 'удален' }} ({{ $event->author_id }})
                        </a>
                    </td>
