...
@section('breadcrumbs', '')
@section('h1')
    Просмотр детальной страницы события №{{$event->id}}
@stop
        ...
        <div class="col-12">
            <div class="row">
                ...
                <div class="col-6">
                    ...
                    <p>
                        <strong>Широта (координаты)</strong>
                        <br>
                        {{$event->lat ?? '-'}}
                    </p>
                    <p>
                        <strong>Долгота (координаты)</strong>
                        <br>
                        {{$event->long ?? '-'}}
                    </p>
                </div>
                <div class="col-6">
                    ...
                    <form class="float-left mr-2" action="{{route('admin.events.destroy', $event)}}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                    ...
                    <form class="float-left mr-2" action="{{route('admin.events.edit', $event)}}" method="GET">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        <button class="btn btn-sm btn-outline-secondary" value="submit" type="submit">
                            Изменить
