...
@section('breadcrumbs', '')
@section('h1')
    Просмотр детальной страницы новости №{{$news->id}}
@stop
    ...
    <div class="row">
        <div class="col-12">
            <div class="row">
                ...
                <div class="col-6">
                    <form class="float-left mr-2" action="{{route('admin.news.destroy', $news)}}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                    ...
                    <form class="float-left mr-2" action="{{route('admin.news.edit', $news)}}" method="GET">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        <button class="btn btn-sm btn-outline-secondary" value="submit" type="submit">
                            Изменить
