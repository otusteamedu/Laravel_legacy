...
@section('pageTitle', __('admin.people.list'))
    ...
    <div class="table-responsive">
            ...
            <table class="table table-striped table-sm" role="list">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin.people.edit', ['person' => $item->id]) }}">{{ $item->id }}</a>
                        </td>
                        <td>{{ $item->name }}</td>
                        <td>{{ $item->sort }}</td>
                        <td>
                            <a href="{{ route('admin.people.edit', ['person' => $item->id]) }}" class="i-icon i-edit text-info">
                                <i class="fas fa-edit"></i>
                                <span>@lang('admin.edit')</span>
                            </a>
                            <a href="{{ route('admin.people.destroy', ['person' => $item->id]) }}"
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
                {{ Form::open(['url' => route('admin.people.index'), 'method' => 'delete', 'class' => 'modal-content']) }}
                    ...
                    <div class="modal-footer">
                        ...
                        {{ Form::submit(__('admin.people.delete'), array('class' => 'btn btn-danger')) }}
                    </div>
                {{ Form::close() }}
            </div>
        </div>
