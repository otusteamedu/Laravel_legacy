...
@section('breadcrumbs', '')
@section('h1')
    Просмотр карточки пользователя №{{$user->id}}
@stop
        ...
        <div class="col-12">
            <div class="row">
                <div class="col-6">
                    ...
                    <p>
                        <strong>Токен восстановления пароля</strong>
                        <br>
                        {{$user->remember_token ?? '-'}}
                    </p>
                    <p>
                ...
                <div class="col-6">
                    <form class="float-left mr-2" action="{{route('admin.users.destroy', $user)}}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                    ...
                    <form class="float-left mr-2" action="{{route('admin.users.edit', $user)}}" method="GET">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        <button class="btn btn-sm btn-outline-secondary" value="submit" type="submit">
                            Изменить
