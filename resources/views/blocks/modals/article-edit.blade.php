    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modal-title">@lang('messages.article-edit.title')</h5>
            ...
            <div class="modal-body">
                {{ Form::open(['url'=> route('articles.update', '#id'), 'method' => 'put', 'id' => 'modal-edit-form']) }}
                    ...
                    <div class="form-group">
                        {{ Form::label('title', trans('messages.title'), ['class'=> 'col-form-label']) }}
                        {{ Form::text('title', null, ['class'=> 'form-control', 'id' => 'modalField-title']) }}
                    </div>
                    <div class="form-group">
                        {{ Form::label('category_id', trans('messages.category'), ['class'=> 'col-form-label']) }}
                        {{ Form::select('category_id', $categoriesList, null, ['placeholder' => 'Не выбрано', 'class'=> 'form-control', 'id' => 'modalField-category_id']) }}
                    </div>
                    <div class="form-group">
                        {{ Form::label('intro_text', trans('messages.intro_text'), ['class'=> 'col-form-label']) }}
                        {{ Form::textarea('intro_text', null, ['class'=> 'form-control', 'id' => 'modalField-intro_text', 'rows' => 3]) }}
                    </div>
                    <div class="form-group">
                        {{ Form::label('full_text', trans('messages.full_text'), ['class'=> 'col-form-label']) }}
